...
class LoadFileForm(FlaskForm):
    file_type = RadioField(_l('Tipo file'), choices=[('image', _l('Foto')), ('video', _l('Video')), ('gpx', _l('Traccia gpx')), ('POI', _('Text/CSV'))], default='image')
    file_name = FileField('File', validators=[FileAllowed(['jpg', 'jpeg', 'webp', 'mov', 'mp4', 'gpx', 'txt', 'csv'])])
...
class AddHotelForm(FlaskForm):
    ...
    geo_long = FloatField()
    check_in_after = TimeField( _('Orari check-in'))
    check_in_before = TimeField()
    check_out_before = TimeField( _('Orario check-out'))
